...
xy = np.loadtxt('./data/600410_data_3.csv', delimiter=',')
#xy = xy[::-1]  # reverse order (chronically ordered)
#xy = xy[:,2:]
x = MinMaxScaler(xy)
y = x[:, [-1]]  # Close as label
'''
yyyy = restore2price(xy,y)
fid = open("price_test.out",'w+')
for i in range(0, len(yyyy)):
    fid.write(str(xy[i][4]))
    fid.write(",")
    fid.write(str(yyyy[i]))
    fid.write('\n')
fid.close()
'''
